...
const Sphere = function () {
    ...
    this.init = function () {
        ...
        this.red_geometry = new BufferGeometry();
        this.red_geometry.setAttribute('position', new BufferAttribute(this.red_vertices, 3).setUsage(DynamicDrawUsage));
        this.red_geometry.setAttribute('color', new BufferAttribute(this.red_colors, 3).setUsage(DynamicDrawUsage));
        this.red_geometry.computeBoundingSphere();
        this.red_geometry.setDrawRange(0, segments);

        this.red_material = new LineBasicMaterial({
            vertexColors: VertexColors,
            blending: AdditiveBlending,
            transparent: true
        });

        this.red_lines_mesh = new LineSegments(this.red_geometry, this.red_material);

        this.group.add(this.lines_mesh);
        this.group.add(this.red_lines_mesh);
            ...
            for (let i = 0; i < this.max_loop; i++) {
                ...
                for (let j = 0; j < this.max_loop; j++) {
                    ...
                    if (data_array != undefined) {
                        v = (this.radius * 3) + (frequencies[j] * mic_sensitivity);
                        vn2 = v * (0.5 + Math.abs(this.perlin.noise(nx, ny, this.noisez)));
                    } else {
                        vn2 = this.radius * (0.5 + Math.abs(this.perlin.noise(nx, ny, this.noisez)));
                    }
                    ...
                    this.red_particles_positions[_index + 0] = x2;
                    this.red_particles_positions[_index + 1] = y2;
                    this.red_particles_positions[_index + 2] = z2;
    ...
    this.update = function (max_distance, mic_sensitivity, frequencies, data_array) {
        ...
        for (let i = 0; i < this.max_loop; i++) {
            ...
            for (let j = 0; j < this.max_loop; j++) {
                ...
                this.red_particles_positions[gl_Index + 0] = x2;
                this.red_particles_positions[gl_Index + 1] = y2;
                this.red_particles_positions[gl_Index + 2] = z2;
    ...
    this.animate = function (max_distance, mic_sensitivity, frequencies, data_array, volume) {
        ...
        let color_pos = 0;
        let num_connected = 0;
        let red_vertex_pos = 0;
        let red_color_pos = 0;
        let red_num_connected = 0;
        ...
        for (let i = 0; i < this.particles_count; i++) {
            ...
            for (let j = i + 1; j < this.particles_count; j++) {
                ...
                let red_dx = this.red_particles_positions[i * 3 + 0] - this.red_particles_positions[j * 3 + 0];
                let red_dy = this.red_particles_positions[i * 3 + 1] - this.red_particles_positions[j * 3 + 1];
                let red_dz = this.red_particles_positions[i * 3 + 2] - this.red_particles_positions[j * 3 + 2];

                let red_dist = Math.sqrt(red_dx * red_dx + red_dy * red_dy + red_dz * red_dz);

                if (red_dist < this.red_max_distance) {

                    let red_alpha = 0.1 - (red_dist / this.red_max_distance) * 0.1;

                    this.red_vertices[ red_vertex_pos++ ] = this.red_particles_positions[i * 3 + 0];
                    this.red_vertices[ red_vertex_pos++ ] = this.red_particles_positions[i * 3 + 1];
                    this.red_vertices[ red_vertex_pos++ ] = this.red_particles_positions[i * 3 + 2];

                    this.red_vertices[ red_vertex_pos++ ] = this.red_particles_positions[j * 3 + 0];
                    this.red_vertices[ red_vertex_pos++ ] = this.red_particles_positions[j * 3 + 1];
                    this.red_vertices[ red_vertex_pos++ ] = this.red_particles_positions[j * 3 + 2];

                    let alpha = volume * 5;
                    this.red_colors[ red_color_pos++ ] = (red_alpha      ) * (alpha);
                    this.red_colors[ red_color_pos++ ] = (red_alpha * 0.1) * (alpha);
                    this.red_colors[ red_color_pos++ ] = (red_alpha * 0.1) * (alpha);
                    this.red_colors[ red_color_pos++ ] = (red_alpha      ) * (alpha);
                    this.red_colors[ red_color_pos++ ] = (red_alpha * 0.1) * (alpha);
                    this.red_colors[ red_color_pos++ ] = (red_alpha * 0.1) * (alpha);
        ...
        this.red_lines_mesh.geometry.setDrawRange(0, red_num_connected);
        this.red_lines_mesh.geometry.attributes.position.needsUpdate = true;
        this.red_lines_mesh.geometry.attributes.color.needsUpdate = true;
